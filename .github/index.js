...
(async () => {
  ...
  if(data.noSecurityHotspot || version >= "8.1" || version < "7.3"){
    ...
    DEFAULT_FILTER="&types=CODE_SMELL, BUG, VULNERABILITY"
  ...
  do {
      try {
          const response = await got(`${sonarBaseURL}/api/rules/search?activation=true&organization=lucideus&ps=${pageSize}&p=${page}${filterRule}`, {
              agent,
              headers
          });
          page++;
...

  {
    ...
    let nbResults;
      ...
      do {
        try {
            const response = await got(`${sonarBaseURL}/api/issues/search?componentKeys=${sonarComponent}&organization=lucideus&ps=${pageSize}&p=${page}&statuses=${OPEN_STATUSES}&resolutions=&s=STATUS&asc=no${leakPeriodFilter}${filterIssue}${withOrganization}`, {
                agent,
                headers
            });
            page++;
